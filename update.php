...
if (isset($_POST['ups'])) {
    ...
    $caretaker = $_POST['caretakerupt'];
    $pregnancyNumber = $_POST['pregnancynoupt'];
    $twinsOrMore = $_POST['twinupt'];
    $weightAtBirth = $_POST['weightupt'];
    $heightAtBirth = $_POST['heightupt'];
   
    $queryyy = "UPDATE vaccination_data SET Care_taker = '$caretaker', Pregnancy_number = '$pregnancyNumber', Twins_or_more = '$twinsOrMore', Weight_at_birth = '$weightAtBirth', Height_at_birth = '$heightAtBirth' WHERE Batchno = $userId";
    ...
    <div class="list-group list-group-flush mx-3 mt-4">
      ...
      <div class="list-group-item py-2">
        <div class="d-flex align-items-center">
          <span class="fw-bold">Electronic CV System</span><br>
          <hr>
        </div>
      </div>
...
$query = "SELECT * FROM vaccination_data WHERE Batchno = $userId LIMIT 1";
$result = mysqli_query($conn, $query);

if ($result && mysqli_num_rows($result) > 0) {
    while($userData = mysqli_fetch_assoc($result)){

    // Populate the form fields in the modal with the retrieved user data
    $childNames = $userData['Child_names'];
    $careTaker = $userData['Care_taker'];
    $pregnancyNumber = $userData['Pregnancy_number'];
    $twinsOrMore = $userData['Twins_or_more'];
    $weightAtBirth = $userData['Weight_at_birth'];
    $heightAtBirth = $userData['Height_at_birth'];
}
}
?>
...
<form method="POST">
      ...
      <div class="input-group-text">Care Taker</div>
     <select name="caretakerupt" class="form-control">
         <option><?php echo @$careTaker  ?></option>
         <option>Yes</option>
          <option>No</option>
         </select>
      ...
      <div class="input-group-text">Pregnancy Number</div>
      <input type="text" class="form-control" placeholder="next vax date..." 
      name="pregnancynoupt" value="<?php echo @$pregnancyNumber;?>" required/>
  

    
      <div class="input-group-text">Twins or More</div>
     <select name="twinupt" class="form-control">
         <option><?php echo @$twinsOrMore; ?></option>
         <option>Yes</option>
          <option>No</option>
         </select>
      ...
      <input type="text" class="form-control" placeholder="next vax date..." 
      name="weightupt" value="<?php echo @$weightAtBirth;?>" required/>


      <div class="input-group-text">Height</div>
      <input type="text" class="form-control" placeholder="next vax date..." 
      name="heightupt" value="<?php echo @$heightAtBirth;?>" required/>
