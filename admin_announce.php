...
?>
                ...
                <button class="btn btn-outline-success btn-sm rounded-3" data-bs-toggle="modal" data-bs-target="#exampleMod"><i class="fas fa-user-plus me-1"></i>&nbsp;Add User
                 ...
                 <a href="" class="btn btn-outline-primary btn-sm rounded-3" data-bs-toggle="modal" data-bs-target="#example"><i class="fas fa-user-plus me-1"></i>&nbsp;Manage</a>
              </div>
            </div>
          </div>
...
<form method="POST" >
                    ...
                    <br>
                    <input type="submit" class="btn btn-outline-secondary form-control" value="Make Admin Now" name="reg">
                </form>
<?php   
   if(isset($_POST['reg'])){
    $user = $_POST['username'];
...
<div class="modal fade" id="example" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog  modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header bg-secondary text-light">
        &nbsp;<h1 class="modal-title fs-5" id="exampleModalLabel"><p style="font-family:roboto;"><i class="fas fa-user-plus me-1"></i>&nbsp;Manage Access</p></h1>
        <button type="button" class="btn-close bg-light" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
<?php
...
if (mysqli_num_rows($q) > 0) {
    ...
    while ($row = mysqli_fetch_array($q)) {
        ...
        <a href='padmin_edit.php?id=".$row['ID']."' class='btn btn-primary btn-sm'><i class='fas fa-trash text-light'>&nbsp;&nbsp;</i>Reset my Password</a></td><tr>";  
}
echo "</table>";
}
else
...
// Now, let's retrieve and display the table
$sql = "SELECT * FROM users WHERE Role='Local User'";
...
<div class="modal fade" id="exampleModaladd" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    ...
    ?>
                    ...
                    <br>
                    <input type="submit" class="btn btn-outline-secondary form-control" value="Register Pediatrician" name="register">
                </form>
           <?php   
              ...
              <div class="card-body">
                <div class="d-flex justify-content-between p-md-1">
                  <div class="d-flex flex-row">
                    <div class="align-self-center">
                      <i class="far fa-bell text-warning fa-3x me-4"></i>
                    </div>
                    <div>
                      <h4>Write an Announcement</h4>
                      <p class="mb-0">
                        <form method="POST">
                        <textarea name="announce" class="form-control col-6"></textarea>
                        <input type="text" name="date" value="<?php echo date('Y-m-d h:i:s'); ?>" class="form-control" readonly/><br>

                        <input type="submit" value="Publish Now" name="pub" class="form-control btn btn-outline-primary col-1">
                      </p></form>
...
if(isset($_POST['pub'])){
    ...
    // Make sure to properly enclose string values in single quotes.
    $sqlid = "INSERT INTO announcements (ID, announces, dates) VALUES (null, '$announce', '$date')";
    $queryi = mysqli_query($conn, $sqlid);
    
    if ($queryi === TRUE) {
        echo "<script>alert('The Announcement was successfully Published')</script>";
    } else {
        echo "<script>alert('Sorry, there was an error Publishing your Announcement')</script>";
    }
}
...
$total=(mysqli_num_rows($prepare));
                    echo $total ."&nbsp; Published" ?></h2>
                  </div>
                </div>
              </div>
            </div>
  ...
  <ul class="pagination pagination-sm">
    <li class="page-item active" aria-current="page">
      <span class="page-link"><i class="fas fa-window-restore me-1"></i>&nbsp;Pick Date </span>
    </li>
